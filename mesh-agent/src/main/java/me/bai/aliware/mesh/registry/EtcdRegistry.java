...
public class EtcdRegistry implements IRegistry {
	...
	public void register(String serviceName, int port) throws Exception {
		...
		String strVal = System.getProperty("size");
		ByteSequence val = ByteSequence.fromString(strVal);     // 目前只需要创建这个key,对应的value暂不使用,先留空
		kv.put(key, val, PutOption.newBuilder().withLeaseId(leaseId).build()).get();
		LOGGER.info("采用etc服务注册发现机制,Register a new service :{}={}", strKey, strVal);
	}
	...
	public List<Endpoint> find(String serviceName) throws Exception {
		...
		LOGGER.info("一共找到{}个endpoint", response.getCount());
		for (com.coreos.jetcd.data.KeyValue kv : response.getKvs()) {
			...
			LOGGER.info("find endpoint:{}", endpointStr);
			String[] ss = endpointStr.split(":");
			Endpoint endpoint = new Endpoint(ss[0], Integer.valueOf(ss[1]));
